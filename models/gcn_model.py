...
class GCN(nn.Module):
    def __init__(self):
        ...
        A_raw = torch.load("E:/vnenv-master (2)/thordata/gcn/obj.dat")
        A = normalize_adj(A_raw).tocsr().toarray()
        self.A = torch.nn.Parameter(torch.Tensor(A))

        objects = open("E:/vnenv-master (2)/thordata/gcn/objects.txt").readlines()
        objects = [o.strip() for o in objects]
        self.n = len(objects)
        self.register_buffer('all_glove', torch.zeros(self.n, 300))

        glove = h5py.File("E:/vnenv-master (2)/thordata/glove_map300d.hdf5","r",)
    ...
    def gcn_embed(self, x, params):
        if params == None:
            ...
            print(self.n)
            print('resnet_embed=', resnet_embed.size())
...
if __name__ == "__main__":
    ...
    #input3 = torch.randn(512,1,1,1000)
    #print(input2)
